...
class TransformationAnalyzer:
    ...
    def __init__(self, workflow: AlteryxWorkflow):
        ...
        self._formula_converter = FormulaConverter()  # Reuse formula_converter for expression conversion
        self._build_graph()
    ...
    def _convert_alteryx_expression(self, expr: str) -> str:
        """Convert Alteryx expression syntax to Trino SQL.

        Delegates to FormulaConverter for consistent expression handling across
        the codebase (CRIT-02 fix: consolidate duplicate conversion logic).
        """
        if not expr:
            return expr
